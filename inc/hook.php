    ...
    function kopa_social_links() {
        ...
        if ( $social_links ) {
            foreach ( $social_links as $k => $v ) {
                ...
                if ( ( 'rss' == $v['id'] && 'HIDE' != $value_theme_mod ) || (! empty($value_theme_mod)) ) {
                    $assing_socials[] = $v;
                }
            }
        }
        ...
        if ( $assing_socials ) {
            echo '<ul class="list-unstyled">';
                foreach ( $assing_socials as $social ) {
                    if ( 'rss' == $social['id'] ) {
                        $rss_value = get_theme_mod($social['id'] . '_url','');
                        if ( empty($rss_value) ) {
                            $social_url =  get_bloginfo('rss2_url');
                        } elseif ( 'HIDE' == $rss_value ) {
                            $social_url = '';
                        } else {
                            $social_url = $rss_value;
                        }
                    } else {
                        $social_url = get_theme_mod($social['id'] . '_url','');
                    }
                    if ( ! empty($social_url) ) {
                        echo sprintf(' <li><a href="%s" class="fa fa-%s" target="_blank" rel="nofollow" title="%s"></a></li>', esc_url($social_url), $social['id'], __('Follow us via ', 'new-lotus-toolkit') . esc_attr($social['title']));
                    }
                }
            echo '</ul>';
        }
